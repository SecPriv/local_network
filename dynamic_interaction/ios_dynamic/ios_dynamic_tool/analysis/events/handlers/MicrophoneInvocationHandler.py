...
class MicrophoneInvocationHandler(FridaEventHandler):
    ...
    def __init__(self, logger: AnalysisEventLogger) -> None:
        ...
        self.requestCount = 0
        self.usage_description_key = "NSMicrophoneUsageDescription"

        logging.info("listening for microphone access...")
    ...
    def handle_event(self, event: FridaEvent):
        ...
        if event['access_mode'] == 'status_check':
            logging.info('Microphone access status checked')
            self.accessStatusChecked = True
            self.evtLogger.log(AccessTypes.Microphone,
                               'access_status_checked', {'event': event})
        elif event['access_mode'] == 'access_request':
            logging.info('Microphone access requested')
    ...
    def report(self, info_plist: dict, app_store_info: dict) -> dict:
        ...
        privacyLabelDict = super().check_privacy_label(
            app_store_info, 'USER_CONTENT', ['Audio Data', 'Audio-Daten'])
